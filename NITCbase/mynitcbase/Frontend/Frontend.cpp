...
{
  // pass the call down to Schema layer
...
{
  // project all attributes — creates a copy of the relation
...
{
  // project only the specified attributes
...
{
  // select records satisfying condition into target relation
...
{
  // step 1: select into .temp
  char tmpRel[ATTR_SIZE];
  strcpy(tmpRel, TEMP);

  int ret = Algebra::select(relname_source, tmpRel, attribute, op, value);
  if (ret != SUCCESS)
  {
    return ret;
  }

  // step 2: open .temp so project can use it
  ret = OpenRelTable::openRel(tmpRel);
  if (ret < 0)
  {
    Schema::deleteRel(tmpRel);
    return ret;
  }

  // step 3: project from .temp into target
  ret = Algebra::project(tmpRel, relname_target, attr_count, attr_list);

  // step 4: close and delete .temp
  OpenRelTable::closeRel(OpenRelTable::getRelId(tmpRel));
  Schema::deleteRel(tmpRel);
